    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{t('enrollment', 'title')}</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Manage teacher assignments and student enrollments
            </p>
          </div>
          <button
        ...
        <motion.div
          ...
          className="flex flex-wrap gap-2 rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 p-4 shadow-sm"
        >
          <button
            onClick={() => setActiveTab('teacher-subjects')}
          ...
          <motion.div
            ...
            >
              ...
              <input
                type="text"
                placeholder={`Search ${
                  activeTab === 'teacher-subjects' ? 'teachers or subjects' : 
                  activeTab === 'student-subjects' ? 'students or subjects' : 'groups'
                }...`}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary transition-all"
              />
            </motion.div>
          )}
            ...
            <div className="rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
              {filteredTeacherSubjects.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  {teacherSubjects.length === 0 && (
                    <p className="text-xs text-muted-foreground mt-1">Click "Assign Teacher" to create a new assignment.</p>
                  )}
                </div>
              ) : (
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                      {filteredTeacherSubjects.map((ts) => (
                        <tr key={ts.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                            {ts.subject ? (
                              <div>
                                <div className="font-medium text-gray-900 dark:text-white">{ts.subject.name}</div>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                            {ts.classroom ? (
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                                {ts.classroom.grade.name} - {ts.classroom.section.label}
                              </span>
                            ) : 'N/A'}
                          </td>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                            {ts.endDate && ts.endDate !== '1970-01-01' && new Date(ts.endDate).getTime() > 0
                              ...
                              : <span className="text-emerald-600 dark:text-emerald-400 font-medium">Ongoing</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {!isLoading && !error && activeTab === 'student-subjects' && (
            <div className="rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-200 dark:border-gray-800">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Student Enrollments</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-gray-800">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Teacher</th>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Classroom</th>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Enrolled On</th>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                      {studentEnrollments.map((enrollment) => (
                        <tr key={enrollment.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                            {enrollment.teacherSubject?.subject ? (
                              <div>
                                <div className="font-medium text-gray-900 dark:text-white">{enrollment.teacherSubject.subject.name}</div>
            ...
            <div className="space-y-4">
              {filteredGroups.length === 0 ? (
                <div className="rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 p-12 text-center">
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-white">No groups found</p>
                </div>
              ) : (
                filteredGroups.map((group) => (
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 p-6 shadow-sm hover:shadow-md transition-all"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-100 dark:border-blue-800">
                            {group.visibility}
                          </span>
                      ...
                      <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {group.members.map((member, idx) => (
                            <div
                              key={idx}
                              className="px-3 py-2 rounded-lg bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-gray-800 text-xs text-gray-700 dark:text-gray-300"
                            >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
          <motion.div
            ...
            className="w-full max-w-md rounded-xl border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 p-6 shadow-xl my-8"
          >
      ...
      {selectedGroup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Add Members to Group</h3>
              <button
                onClick={() => {
                  setSelectedGroup(null);
                  setGroupStudentIds([]);
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {students.map((student) => (
                <label
                  key={student.id}
                  className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer transition-colors"
                >
                  <input
                    type="checkbox"
